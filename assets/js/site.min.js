jQuery(document).ready(function($){
    ...
    $('#second-index select[name="store-oid"], #second-edit select[name="store-oid"]').change(function(){
        $.get($base + 'second/first/' + $(this).val(), function(data){
            var f = $('select[name="flc-oid"]');
            var hf = $('input[name="flc-oid"]');
            f.empty();
            if (data.status == 0) {
                $.each(data.data, function(key, val){
                    if (hf.val() == key) {
                        f.append($('<option>').text(val).val(key).attr('selected', true));
                    } else {
                        f.append($('<option>').text(val).val(key));
                    }
                });
            }
        }, "json");
    ...
    $("#dish-edit form").submit(function(){
        ...
        var id = $("input[type='hidden'][name='id']").val();
        var data = _.filter($(this).serializeArray(), function(item){ return (_.indexOf(['store-oid', 'flc-oid', 'slc-oid'], item.name) == -1); });
        $(this).find('[name="store-oid"]').each(function() {
            if ($(this).is(':checked')) {
                data.push({name: 'store-oid[]', value: $(this).val()});
                data.push({name: 'flc-oid[]', value: $(this).parent().parent().find('select[name="flc-oid"]').val()});
                data.push({name: 'slc-oid[]', value: $(this).parent().parent().find('select[name="slc-oid"]').val()});
            }
        });
    ...
    $('#dish-index select[name="store-oid"]').change(function(){
        $.get($base + 'dish/first/' + $(this).val(), function(data){
            var f = $('select[name="flc-oid"]');
            var hf = $('input[name="flc-oid"]');
            f.empty();
            if (data.status == 0) {
                ...
                $('#dish-index select[name="flc-oid"], #dish-edit select[name="flc-oid"]').trigger('change');
            }
        }, "json");
    }).trigger('change');
    $('#dish-index select[name="flc-oid"], #dish-edit select[name="flc-oid"]').change(function(){
        var $this = $(this);
        $.get($base + 'dish/second/' + $(this).val(), function(data){
            var s = $this.parent().find('select[name="slc-oid"]');
            var hf = $this.parent().find('input[name="slc-oid"]');
            s.empty();
            if (data.status == 0) {
                $.each(data.data, function(key, val){
                    if (hf.val() == key) {
    ...
    $('#dish-edit [name="store-oid"]').click(function(){
        if ($(this).is(':checked')) {
    ...
    $('#dish-edit [name="flc-oid"]').trigger('change');
