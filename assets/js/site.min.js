jQuery(document).ready(function($){
    ...
    $('#dish-index-content .edit-category').click(function(){
        ...
        $.get($base + 'dish/category/' + id, function(data){
            if (data.status == 0) {
                $('#dish-modal-category .model-enable').html(_.template($('#tpl-category').html(), {data: data.data}));
    ...
    $('#dish-modal-category .add').click(function(){
        var data = {
            'first': $('#dish-modal-category select[name="flc-oid"]').val(),
            'fname': $('#dish-modal-category select[name="flc-oid"]').find("option:selected").text(),
            'second': $('#dish-modal-category select[name="slc-oid"]').val(),
            'sname': $('#dish-modal-category select[name="slc-oid"]').find("option:selected").text()
        };
        console.log(data);
        if (data.first == '0' || !data.second || data.second == '0') {
            alert('请选择一级分类和二级分类');
            return false;
        }
        var has = false;
        $('#dish-modal-category .model-enable p').each(function() {
            if ($(this).attr('data-fid') == data.first && $(this).attr('data-sid') == data.second) {
                has = true;
                alert('已经添加了，手不能抖呀～');
                return false;
            }
        ...
        if (!has) {
            $('#dish-modal-category .model-enable').append(_.template($('#tpl-category').html(), {data: [data]}))
        }
    });
    $('#dish-modal-category').on('click', 'span.delete', function() {
        $(this).parent().remove();
    });
    $('#dish-modal-category .save').click(function(){
        ...
        var data = [];
        $('#dish-modal-category .model-enable p').each(function(){
            data.push({'first': $(this).attr('data-fid'), 'second': $(this).attr('data-sid')});
        });
        $.post($base + 'dish/savecategory/', {id: id, data: data}, function(data) {
            alert(data.msg);
            if (data.status == 0) {
